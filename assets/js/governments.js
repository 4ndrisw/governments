// Init single government
function init_government(userid) {
    load_small_table_item(userid, '#government', 'governmentid', 'governments/get_government_data_ajax', '.table-governments');
}


// Validates government add/edit form
function validate_government_form(selector) {

    selector = typeof (selector) == 'undefined' ? '#government-form' : selector;
    ...
    $("body").find('input[name="number"]').rules('add', {
        remote: {
            url: admin_url + "governments/validate_government_number",
            type: 'post',
            data: {
                number: function () {
                ...
                date: function () {
                    return $('body').find('.government input[name="date"]').val();
                },
            }
        },
        messages: {
            remote: app.lang.government_number_exists,
        }
    });
...
function get_government_item_preview_values() {
    var response = {};
...
function add_government_item_to_table(data, itemid){

  // If not custom data passed get from the preview
  data = typeof (data) == 'undefined' || data == 'undefined' ? get_government_item_preview_values() : data;
...
// From government table mark as
function government_mark_as(state_id, government_id) {
    var data = {};
    data.state = state_id;
    data.governmentid = government_id;
    $.post(admin_url + 'governments/update_government_state', data).done(function (response) {
        //table_governments.DataTable().ajax.reload(null, false);
        reload_governments_tables();
    });
}

// Reload all governments possible table where the table data needs to be refreshed after an action is performed on task.
function reload_governments_tables() {
    var av_governments_tables = ['.table-governments', '.table-rel-governments'];
    $.each(av_governments_tables, function (i, selector) {
