...
<div class="modal fade email-template" data-editor-id=".<?php echo 'tinymce-'.$government->id; ?>" id="government_send_to_client_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<?php echo form_open('admin/governments/send_to_email/'.$government->id); ?>
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close close-send-template-modal"><span aria-hidden="true">&times;</span></button>
				<h4 class="modal-title" id="myModalLabel">
					<span class="edit-title"><?php echo _l('government_send_to_client_modal_heading'); ?></span>
				</h4>
			</div>
				...
				<div class="row">
					<div class="col-md-12">
						<div class="form-group">
							...
							$selected = array();
							$contacts = $this->clients_model->get_contacts($government->clientid,array('active'=>1,'government_emails'=>1));
							foreach($contacts as $contact){
								array_push($selected,$contact['id']);
							}
							 if(count($selected) == 0){
                                    echo '<p class="text-danger">' . _l('sending_email_contact_permissions_warning',_l('customer_permission_government')) . '</p><hr />';
                                }
							echo render_select('sent_to[]',$contacts,array('id','email','firstname,lastname'),'invoice_government_sent_to_email',$selected,array('multiple'=>true),array(),'','',false);
							?>
						</div>
						...
						<div class="checkbox checkbox-primary">
							...
							<label for="attach_pdf"><?php echo _l('government_send_to_client_attach_pdf'); ?></label>
						</div>
						<h5 class="bold"><?php echo _l('government_send_to_client_preview_template'); ?></h5>
						<hr />
						<?php echo render_textarea('email_template_custom','',$template->message,array(),array(),'','tinymce-'.$government->id); ?>
						<?php echo form_hidden('template_name',$template_name); ?>
					</div>
				</div>

				<?php if(count($government->attachments) > 0){ ?>
					<hr />
					<div class="row">
						<div class="col-md-12">
							<h5 class="bold no-margin"><?php echo _l('include_attachments_to_email'); ?></h5>
							<hr />
							<?php foreach($government->attachments as $attachment) { ?>
								<div class="checkbox checkbox-primary">
