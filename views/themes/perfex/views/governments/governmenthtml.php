            ...
            <div class="col-md-12">
               ...
               <div class="col-md-9">         
                  <?php
                     ...
                     if ($government->state != 4 && $government->state != 3 && $government->state != 5) {
                       $can_be_accepted = true;
                       if($identity_confirmation_enabled == '0'){
                         echo form_open($this->uri->uri_string(), array('class'=>'pull-right mtop7 action-button'));
                         echo form_hidden('government_action', 4);
                         echo '<button type="submit" data-loading-text="'._l('wait_text').'" autocomplete="off" class="btn btn-success action-button accept"><i class="fa fa-check"></i> '._l('clients_accept_government').'</button>';
                         echo form_close();
                       } else {
                         echo '<button type="button" id="accept_action" class="btn btn-success mright5 mtop7 pull-right action-button accept"><i class="fa fa-check"></i> '._l('clients_accept_government').'</button>';
                       }
                     } else if($government->state == 3){
                       if (($government->duedate >= date('Y-m-d') || !$government->duedate) && $government->state != 5) {
                         $can_be_accepted = true;
                         if($identity_confirmation_enabled == '0'){
                           echo form_open($this->uri->uri_string(),array('class'=>'pull-right mtop7 action-button'));
                           echo form_hidden('government_action', 4);
                     ...
                     if ($government->state != 4 && $government->state != 3 && $government->state != 5) {
                       ...
                       echo form_hidden('government_action', 3);
                       echo '<button type="submit" data-loading-text="'._l('wait_text').'" autocomplete="off" class="btn btn-default action-button accept"><i class="fa fa-remove"></i> '._l('clients_decline_government').'</button>';
                       echo form_close();
                     }
                     ?>
                  <?php echo form_open(site_url('governments/pdf/'.$government->id), array('class'=>'pull-right action-button')); ?>
                  ...
                  <?php echo form_close(); ?>
                  <?php if((is_client_logged_in() && has_contact_permission('governments'))  || is_staff_member()){ ?>
      ...
      <div class="col-md-10 col-md-offset-1">
         <div class="row mtop20">
            ...
            <div class="col-sm-6 text-right transaction-html-info-col-right">
               <span class="bold government_to"><?php echo _l('government_to'); ?>:</span>
         ...
         <div class="row">
            ...
            <?php if(!empty($government->clientnote)){ ?>
            <div class="col-md-12 government-html-note">
            <hr />
               <b><?php echo _l('government_order'); ?></b><br /><?php echo $government->clientnote; ?>
            </div>
            <?php } ?>
            <?php if(!empty($government->terms)){ ?>
            <div class="col-md-12 government-html-terms-and-conditions">
               <b><?php echo _l('terms_and_conditions'); ?>:</b><br /><?php echo $government->terms; ?>
            </div>
            <?php } ?>
...
<?php
   if($identity_confirmation_enabled == '1' && $can_be_accepted){
    get_template_part('identity_confirmation_form',array('formData'=>form_hidden('government_action',4)));
   }
   ?>
<script>
   $(function(){
